...
export async function GET(request) {
  try {
    ...
    const acceptedRecommendations =
      interactions?.filter((i) => i.interaction_type === "accept").length || 0;
    const rejectedRecommendations =
      interactions?.filter((i) => i.interaction_type === "reject").length || 0;
    ...
    const avgConfidenceScore =
      interactions?.length > 0
        ? interactions.reduce(
            (sum, i) => sum + (i.smart_recommendations?.confidence_score || 0),
            0,
          ) / interactions.length
        : 0;
    ...
    const acceptedRecs =
      interactions?.filter((i) => i.interaction_type === "accept") || [];
