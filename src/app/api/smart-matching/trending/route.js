...
export async function GET(request) {
  try {
    ...
    const marketSummary = {
      ...
      explosive_momentum: sortedTrending.filter(p => p.momentum === 'Explosive').length,
      hot_programs: sortedTrending.filter(p => p.momentum === 'Hot').length,
      emerging_opportunities: sortedTrending.filter(p => p.momentum === 'Emerging').length,
      average_trending_score: (sortedTrending.reduce((sum, p) => sum + p.trending_score, 0) / sortedTrending.length).toFixed(1),
      critical_windows: sortedTrending.filter(p => p.opportunity_window.includes('12-24 hours')).length,
      low_risk_opportunities: sortedTrending.filter(p => p.risk_level === 'Low' || p.risk_level === 'Very Low').length,
    ...
    // Add AI insights
    const aiInsights = {
      top_recommendation: sortedTrending[0]?.program_name || 'No trending programs',
      market_temperature: 'Hot - Multiple opportunities detected',
      timing_advice: 'Act on top 2 programs within 24 hours for maximum benefit',
      risk_assessment: 'Low overall risk - favorable market conditions',
      strategy_tip: 'Focus on programs with "Explosive" or "Hot" momentum for quick wins'
    };
