...
export async function GET(request) {
  try {
    ...
    let filteredData = liveCommissionData.filter(
      (program) => program.current_commission >= minCommission,
    );
    ...
    switch (sortBy) {
      case "commission":
        filteredData.sort(
          (a, b) => b.current_commission - a.current_commission,
        );
        break;
      case "trending_score":
      ...
      case "epc":
        filteredData.sort(
          (a, b) => parseFloat(b.estimated_epc) - parseFloat(a.estimated_epc),
        );
        break;
      case "health":
    ...
    const marketInsights = {
      ...
      average_commission: (
        filteredData.reduce((sum, p) => sum + p.current_commission, 0) /
        filteredData.length
      ).toFixed(1),
      highest_commission: Math.max(
        ...filteredData.map((p) => p.current_commission),
      ),
      market_trend: Math.random() > 0.5 ? "rising" : "stable",
      hot_programs: filteredData.filter((p) => p.trending_score > 85).length,
      seasonal_opportunities: filteredData.filter((p) => p.seasonal_bonus)
        .length,
