...
async function handleUserBehaviorTracking(data) {
  const { user_id, session_id, action_type, target_type, target_id, metadata } = data;
...
async function handleRecommendationFeedback(data) {
  ...
  if (error) {
    throw new Error(`Failed to update recommendation feedback: ${error.message}`);
  }
...
async function handleBulkDataSync(data) {
  ...
  for (const record of records) {
    try {
      ...
      if (error) {
        results.push({ record: record.id || 'unknown', success: false, error: error.message });
      } else {
        results.push({ record: record.id || 'unknown', success: true, data: inserted });
      }
    } catch (error) {
      results.push({ record: record.id || 'unknown', success: false, error: error.message });
    }
...
async function updateAffiliateScores(affiliateId) {
  ...
  if (performance && performance.length > 0) {
    ...
    const avgClicks = performance.reduce((sum, p) => sum + p.clicks, 0) / performance.length;
    ...
    const avgRetention = performance.reduce((sum, p) => sum + p.retention_rate, 0) / performance.length;
    ...
    const newReputationScore = Math.round((conversionRate * 30 + revenueScore * 0.4 + retentionScore * 0.3));
