    ...
    try {
      ...
      await supabase
        ...
        .insert([{
          ...
          user_id: userId,
          interaction_type: rec.is_accepted ? 'accept' : (rec.feedback === -1 ? 'reject' : 'view'),
          metadata: { feedback: rec.feedback, is_accepted: rec.is_accepted }
        }]);

      results.push({ recommendation_id: rec.recommendation_id, success: true, data: updatedRec });
    } catch (error) {
      results.push({ recommendation_id: rec.recommendation_id, success: false, error: error.message });
    }
  }

  return { operation: 'bulk_recommendation_feedback', processed: results.length, results };
}
...
async function handleBulkCasinoImport(userId, data) {
  ...
  for (const casino of casinos) {
    try {
      ...
      results.push({ casino: casino.name, success: false, error: error.message });
    }
  }

  return { operation: 'bulk_casino_import', processed: results.length, results };
}
...
async function handleBulkAffiliateImport(userId, data) {
  ...
  for (const affiliate of affiliates) {
    try {
      ...
      results.push({ affiliate: affiliate.name, success: true, data: newAffiliate });
    } catch (error) {
      results.push({ affiliate: affiliate.name, success: false, error: error.message });
    }
  }

  return { operation: 'bulk_affiliate_import', processed: results.length, results };
}
...
async function handlePerformanceSummary(userId, data) {
  ...
  const acceptedRecs = interactions.filter(i => i.interaction_type === 'accept').length;
  const rejectedRecs = interactions.filter(i => i.interaction_type === 'reject').length;
  const acceptanceRate = totalRecs > 0 ? (acceptedRecs / totalRecs) * 100 : 0;

  // Average confidence score
  const avgConfidence = totalRecs > 0
    ? recommendations.reduce((sum, r) => sum + r.confidence_score, 0) / totalRecs
    : 0;
