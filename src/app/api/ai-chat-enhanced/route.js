...
class EnhancedAIChatAssistant {
  constructor() {
    this.affiliatePersona = {
      ...
      personality:
        "Professional, data-driven, encouraging, and results-focused",
  ...
  async generateEnhancedResponse(query, context = {}) {
    try {
      ...
      const enhancedResponse = await this.enhanceWithAffiliateContext(
        aiResponse,
        query,
        context,
      );
    ...
    try {
      ...
      if (response.ok) {
        ...
        return (
          ...
          "I understand your affiliate marketing question and I'm here to help!"
        );
      }
    } catch (error) {
  ...
  async enhanceWithAffiliateContext(aiResponse, originalQuery, context) {
    ...
    if (
      ...
      originalQuery.toLowerCase().includes("program")
    ) {
      enhancedResponse +=
    ...
    if (
      originalQuery.toLowerCase().includes("content") ||
      originalQuery.toLowerCase().includes("blog")
    ) {
      enhancedResponse +=
