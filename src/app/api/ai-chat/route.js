...
export async function POST(request) {
  try {
    ...
    if (intent.needsSmartMatching && user_id) {
      platformIntegration = await generateSmartMatchingIntegration(
        intent,
        user_id,
      );
...
function analyzeUserIntent(message) {
  ...
  if (
    lowerMessage.includes("recommend") ||
    lowerMessage.includes("best program") ||
    lowerMessage.includes("which affiliate") ||
    lowerMessage.includes("find program") ||
  ...
  if (
    ...
    lowerMessage.includes("flowlync")
  ) {
...
async function generateSmartMatchingIntegration(intent, userId) {
  return {
    ...
    description:
      "Based on your question, I can find the perfect affiliate programs for you using our AI Smart Matching system.",
    action: {
      type: "smart_matching",
      endpoint: `/api/smart-matching/recommendations?user_id=${userId}&limit=5`,
      button_text: "Find My Perfect Programs",
      follow_up:
        "This will analyze your traffic, audience, and goals to recommend the highest-converting casino affiliate programs for your specific situation.",
    },
    quick_setup: {
      message:
        "Don't have a profile yet? I can help you set one up in 2 minutes to get better recommendations.",
...
function generatePlatformGuidance(intent) {
  ...
  guidance.features.push({
    name: "Smart Matching",
    description:
      "AI-powered system that finds the best affiliate programs for your specific situation",
    link: "/smart-matching",
  ...
  guidance.features.push({
    ...
    description:
      "Get expert advice anytime with our specialized affiliate marketing AI",
    link: "#chat",
...
function generateUserProfilingQuestions(intent) {
  return {
    ...
    benefit:
      "This helps me provide more targeted advice and better affiliate program recommendations.",
  };
}
