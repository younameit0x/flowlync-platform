...
class DataQualityVerification {
  ...
  async verifyDataQuality(records) {
    console.log(`🔍 Starting data quality verification for ${records.length} records...`);
    ...
    for (const record of records) {
      try {
        ...
        const verificationResult = await this.performComprehensiveVerification(record);
        ...
        console.error(`❌ Verification error for ${record.name}:`, error.message);
    ...
    console.log(`✅ Data quality verification complete: ${verificationResults.length} records processed`);
  ...
  async performComprehensiveVerification(record) {
    ...
    for (const [checkName, algorithm] of Object.entries(this.verificationAlgorithms)) {
      try {
    ...
    verificationResult.quality_score = checksPerformed > 0 ? 
      Math.round(totalScore / checksPerformed) : 0;
  ...
  async verifyWebsiteAccessibility(record) {
    try {
      ...
      const isAccessible = this.qualityRules.website.expected_status.includes(statusCode);
      ...
      return {
        ...
        issue: !isAccessible ? `Website not accessible (${statusCode})` : 
               (!hasCasinoContent ? 'No casino content detected' : null)
      };
  ...
  async verifyAffiliateProgramExists(record) {
    try {
      const page = await this.browser.newPage();
      await page.goto(record.website, { waitUntil: 'networkidle0', timeout: 15000 });
      ...
      const affiliateLinks = await page.$$eval('a', anchors => 
        anchors.filter(anchor => {
          ...
          return text.includes('affiliate') || text.includes('partner') ||
                 href.includes('affiliate') || href.includes('partner');
        }).map(anchor => ({
      ...
      if (affiliateLinks.length > 0) {
        try {
          await page.goto(affiliateLinks[0].href, { waitUntil: 'networkidle0', timeout: 10000 });
          affiliatePageContent = await page.content();
          affiliatePageExists = this.containsAffiliateContent(affiliatePageContent);
        } catch (e) {
      ...
      const hasAffiliateProgram = affiliateLinks.length > 0 && affiliatePageExists;
  ...
  async verifyCommissionAccuracy(record) {
    ...
    // Basic range check
    const withinRange = commissionRate >= this.qualityRules.commission_rate.min_value &&
    ...
    return {
      ...
      details: {
        ...
        industry_comparison: commissionRate > industryAverage ? 'above_average' : 'below_average'
      },
      issue: !isAccurate ? `Commission rate ${commissionRate}% appears inaccurate` : null,
  ...
  async verifyContactInformation(record) {
    ...
    const contactScore = hasValidContact ? 
      Math.min(100, (validEmails / Math.max(1, totalEmails)) * 100) : 0;
    ...
    return {
      ...
      details: {
        ...
        contact_types: Object.keys(contactInfo).filter(key => contactInfo[key].length > 0)
      },
  ...
  async verifyLicenseAuthenticity(record) {
    ...
    const isKnownAuthority = this.qualityRules.license.known_authorities.some(
      authority => regulatoryBody.toLowerCase().includes(authority.toLowerCase())
    );
    
    // License number format validation
    let validLicenseFormat = false;
    if (record.license_number) {
      validLicenseFormat = this.validateLicenseFormat(record.license_number, regulatoryBody);
    }
    
    const isAuthentic = isKnownAuthority && (validLicenseFormat || !record.license_number);
    ...
    return {
      ...
      issue: !isAuthentic ? `Unrecognized licensing authority: ${regulatoryBody}` : null,
  ...
  async verifyOperationalStatus(record) {
    try {
      ...
      const hasRegistrationForm = $('form').text().toLowerCase().includes('register') ||
                                  $('form').text().toLowerCase().includes('sign up');
      const hasGames = content.toLowerCase().includes('slots') || 
                      content.toLowerCase().includes('blackjack') ||
                      content.toLowerCase().includes('roulette');
      const hasPromotion = content.toLowerCase().includes('bonus') ||
                          content.toLowerCase().includes('promotion');
      
      // Check for maintenance or closure notices
      const maintenanceKeywords = ['maintenance', 'closed', 'suspended', 'temporarily unavailable'];
      ...
      const operationalScore = [hasLoginForm, hasRegistrationForm, hasGames, hasPromotion]
      ...
      return {
        ...
        score: isOperational ? Math.max(operationalScore, 75) : operationalScore,
        details: {
  ...
  async detectDuplicates(record) {
    try {
      ...
      return {
        ...
        issue: hasDuplicates ? `${duplicates.length} potential duplicates found` : null,
        severity: 'medium'
      };
  ...
  async checkDataFreshness(record) {
    const now = new Date();
    const recordDate = new Date(record.discovered_at || record.created_at || now);
    ...
    return {
      ...
      issue: outdated ? `Data is ${ageInDays} days old and may be outdated` : null,
  ...
  async enrichRecordData(record) {
    ...
    try {
      ...
      enhancements.website_metadata = await this.extractWebsiteMetadata(record.website);
      ...
      // Technical details
      enhancements.technical_details = await this.extractTechnicalDetails(record.website);
  ...
  containsCasinoContent(content) {
    const casinoKeywords = [
      'casino', 'slots', 'blackjack', 'roulette', 'poker', 'baccarat',
      'gambling', 'bet', 'gaming', 'jackpot', 'bonus', 'deposit'
    ];
  ...
  containsAffiliateContent(content) {
    const affiliateKeywords = [
      'affiliate', 'partner', 'commission', 'revenue share', 'earn',
      'referral', 'marketing', 'promotion', 'payout'
    ];
  ...
  extractPhones(record) {
    ...
    while ((match = phoneRegex.exec(textToSearch)) !== null) {
      if (match[0].length >= 10) { // Minimum phone number length
        phones.push(match[0]);
      }
    }
  ...
  extractAddresses(record) {
    ...
    const addresses = [];
    const addressRegex = /\d+\s+[A-Za-z\s,]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd)/g;
  ...
  calculateSimilarity(record1, record2) {
    ...
    const overallScore = (criteria.name + criteria.website + criteria.email) / 3;
    
    return {
      score: overallScore,
      is_duplicate: overallScore >= this.qualityRules.duplicate_detection.name_similarity_threshold,
      matching_criteria: Object.keys(criteria).filter(key => criteria[key] > 0.7)
    };
  }
  ...
  generateVerificationSummary(results) {
    ...
    const verifiedRecords = results.filter(r => r.verification_status === 'verified').length;
    const partiallyVerifiedRecords = results.filter(r => r.verification_status === 'partially_verified').length;
    const failedRecords = results.filter(r => r.verification_status === 'failed').length;
    
    const averageQualityScore = results.reduce((sum, r) => sum + r.quality_score, 0) / totalRecords;
    ...
    return {
      ...
      verification_rate: ((verifiedRecords + partiallyVerifiedRecords) / totalRecords * 100).toFixed(1),
  ...
  async storeVerificationResults(results, summary) {
    try {
      ...
      console.log(`✅ Stored verification results for ${results.length} records`);
  ...
  async function testDataQualityVerification() {
    ...
    const testRecords = [
      ...
      },
      {
        ...
        discovered_at: new Date(Date.now() - 60 * 24 * 60 * 60 * 1000).toISOString() // 60 days old
      }
    ];
