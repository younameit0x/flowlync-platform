...
class AIAffiliateProgramDetector {
  ...
  async detectAffiliateProgram(casino) {
    ...
    try {
      ...
      if (mainPageResults.affiliateUrl) {
        affiliatePageResults = await this.scanAffiliatePage(
          page,
          mainPageResults.affiliateUrl,
          casino,
        );
      }
  ...
  async scanMainPage(page, casino) {
    ...
    try {
      ...
      await page.goto(casino.website, {
      ...
      for (const link of links) {
        if (
          this.matchesAffiliatePattern(link.href) ||
          this.matchesAffiliateText(link.text)
        ) {
  ...
  async scanAffiliatePage(page, affiliateUrl, casino) {
    ...
    try {
      ...
      results.applicationProcess = this.analyzeApplicationProcess(
        results.content,
      );
  ...
  async performAIAnalysis(mainPageResults, affiliatePageResults, casino) {
    ...
    const maxPossibleScore = Object.values(this.confidenceWeights).reduce(
      (a, b) => a + b,
      0,
    );
    const normalizedConfidence = Math.min(
      100,
      (totalConfidence / maxPossibleScore) * 100,
    );
    ...
    const predictedData = this.predictMissingData(
      mainPageResults,
      affiliatePageResults,
      casino,
    );
    ...
    const affiliateProgram = {
      ...
      commission_rates: affiliatePageResults
        ? affiliatePageResults.commissionRates
        : [],
      ...
      // Contact information
      contact_emails: this.combineContactInfo(
        mainPageResults,
        affiliatePageResults,
      ),
      phone_numbers: affiliatePageResults
        ? affiliatePageResults.phoneNumbers
        : [],

      // Terms and conditions
      terms_and_conditions: affiliatePageResults
        ? affiliatePageResults.terms
        : {},

      // Application process
      application_process: affiliatePageResults
        ? affiliatePageResults.applicationProcess
        : {},
      ...
      extraction_completeness: this.calculateCompleteness(
        mainPageResults,
        affiliatePageResults,
      ),
    };
    ...
    console.log(
      `✅ AI analysis complete - Confidence: ${normalizedConfidence.toFixed(1)}%`,
    );
  ...
  matchesAffiliatePattern(url) {
    return this.aiPatterns.affiliateUrlPatterns.some((pattern) =>
      pattern.test(url),
    );
  }
  ...
  generateAIInsights(mainPageResults, affiliatePageResults, casino) {
    ...
    if (
      affiliatePageResults &&
      affiliatePageResults.commissionRates.length > 0
    ) {
      insights.push(
        "Established affiliate program with clear commission structure",
      );
    } else if (mainPageResults.hasAffiliateSection) {
      insights.push(
        "Basic affiliate program detected, may require manual verification",
      );
    }

    // Analyze communication channels
    const totalContacts = this.combineContactInfo(
      mainPageResults,
      affiliatePageResults,
    ).length;
    if (totalContacts > 2) {
      insights.push(
        "Multiple contact channels available for affiliate support",
      );
    } else if (totalContacts === 0) {
      insights.push(
        "Limited contact information available, may indicate private program",
      );
    }

    // Analyze content quality
    const contentLength = (
      mainPageResults.mainPageContent +
      (affiliatePageResults ? affiliatePageResults.content : "")
    ).length;
  ...
  assessDataQuality(mainPageResults, affiliatePageResults) {
    ...
    if (
      affiliatePageResults &&
      Object.keys(affiliatePageResults.terms).length > 0
    )
      score++;
    ...
    if (
      affiliatePageResults &&
      affiliatePageResults.applicationProcess.hasApplication
    )
      score++;
  ...
  determineCommissionType(affiliatePageResults) {
    ...
    if (
      affiliatePageResults.commissionRates.length > 0 &&
      affiliatePageResults.cpaRates.length > 0
    ) {
      return "hybrid";
  ...
  identifyTermType(termText) {
    if (
      termText.toLowerCase().includes("payout") ||
      termText.toLowerCase().includes("payment")
    ) {
      return "payment_terms";
    }
    if (
      termText.toLowerCase().includes("cookie") ||
      termText.toLowerCase().includes("tracking")
    ) {
      return "tracking_duration";
    }
    if (
      termText.toLowerCase().includes("geo") ||
      termText.toLowerCase().includes("country")
    ) {
  ...
  analyzeApplicationProcess(content) {
    const hasApplication = this.aiPatterns.applicationPatterns.some((pattern) =>
  ...
  extractProcessSteps(content) {
    ...
    const steps = [];
    const stepPatterns = [/step\s*\d+[:\-]\s*([^.]+)/gi, /\d+\.\s*([^.]+)/g];
  ...
  async processCasinoBatch(casinos) {
    ...
    this.detectionStats.confidenceScore =
      results.reduce(
        (sum, result) => sum + parseFloat(result.confidence_score),
        0,
      ) / results.length || 0;

    console.log(
      `✅ Batch processing complete: ${results.length} affiliate programs detected`,
    );
    return results;
  }
