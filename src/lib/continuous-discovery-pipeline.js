...
} = require("./comprehensive-data-collector");
...
class ContinuousDiscoveryPipeline {
  ...
  async stopPipeline() {
    ...
    await this.logPipelineEvent("pipeline_stopped", {
      ...
      uptime:
        Date.now() -
        (new Date(this.pipelineStats.lastExecution).getTime() || Date.now()),
    });
  ...
  scheduleJobs() {
    ...
    this.scheduledJobs.set(
      ...
      cron.schedule(
        this.config.schedules.comprehensive_collection,
        async () => {
    ...
    this.scheduledJobs.set(
      "automated_discovery",
      cron.schedule(
        this.config.schedules.automated_discovery,
        async () => {
    ...
    this.scheduledJobs.set(
      "ai_affiliate_detection",
      cron.schedule(
        this.config.schedules.ai_affiliate_detection,
        async () => {
          await this.executeJob("ai_affiliate_detection", async () => {
            ...
            if (
              newCasinos.length >=
              this.config.thresholds.new_casinos_for_ai_detection
            ) {
              return await this.modules.aiDetector.processCasinoBatch(
                newCasinos,
              );
            }
    ...
    this.scheduledJobs.set(
      "regional_coverage",
      cron.schedule(
        this.config.schedules.regional_coverage,
        async () => {
    ...
    this.scheduledJobs.set(
      "data_verification",
      cron.schedule(
        this.config.schedules.data_verification,
        async () => {
          await this.executeJob("data_verification", async () => {
            ...
            if (oldRecords.length > 0) {
              return await this.modules.dataVerification.verifyDataQuality(
                oldRecords,
              );
            }
    ...
    this.scheduledJobs.set(
      "incremental_updates",
      cron.schedule(
        this.config.schedules.incremental_updates,
        async () => {
    ...
    this.scheduledJobs.set(
      "health_check",
      cron.schedule(
        this.config.schedules.health_check,
        async () => {
  ...
  async getRecordsForVerification() {
    try {
      const cutoffDate = new Date(
        Date.now() - this.config.thresholds.data_age_for_verification,
      );
      ...
      console.error(
        "❌ Error fetching records for verification:",
        error.message,
      );
      return [];
    }
  ...
  updateAverageExecutionTime(executionTime) {
    ...
    this.pipelineStats.averageExecutionTime =
      (currentAverage * (totalExecutions - 1) + executionTime) /
      totalExecutions;
  }
  ...
  async checkForAlerts(metrics) {
    ...
    if (metrics.average_execution_time > 300000) {
      // 5 minutes
      alerts.push({
        type: "slow_execution",
        severity: "warning",
  ...
  async storeHealthCheck(health) {
    try {
      await supabase.from("system_health_checks").insert(health);
    } catch (error) {
